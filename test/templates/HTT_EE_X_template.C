...
/**
   \class   HTT_EE_X_template HTT_EE_X_template.C "HiggsAnalysis/HiggsToTauTau/postfit/templates/HTT_EE_X_template.C"
...
#else
  if(std::string(inputfile).find("7TeV")!=std::string::npos){dataset = "CMS (unpublished), 4.9 fb^{-1} at 7 TeV";}
  if(std::string(inputfile).find("8TeV")!=std::string::npos){dataset = "CMS (unpublished), 19.7 fb^{-1} at 8 TeV";}
#endif
