...
int 
findLastBin(TH1F* h,double frac=0.1)
{
  double max=h->GetMaximum();
  int bin=0;
...
void 
findRebin(TString * Input,Int_t N, Int_t * Rebin)
{  
  float binw=0.;
...
// from addfitnuisance.C felix
//Rebin the histogram to the first ones
...
void 
sobWeightedCombineMSSM(TString *Input, TString outName, int weight=1, float  MuValue=1.0)
{  
  ...
  for(Int_t f=0;f<n;f++){
    ...
    cout << "bins for ggH " << ggH->GetNbinsX() << endl;
    cout << "bins for Ztt " << Ztt->GetNbinsX() << endl;
    signal->Add(Ztt,-1);
  ...
  for(Int_t f=1;f<n;f++){
    ...
    const int lNBins = ggH->GetNbinsX();
    double *lAxis    = getAxis(ggH);
    ///Rebin
    ggH2      = rebin(ggH2     ,lNBins,lAxis);
    signal2   = rebin(signal2  ,lNBins,lAxis);
    Ztt2      = rebin(Ztt2     ,lNBins,lAxis);
    data_obs2 = rebin(data_obs2,lNBins,lAxis);
    TT2       = rebin(TT2      ,lNBins,lAxis);
    EWK2      = rebin(EWK2     ,lNBins,lAxis);
    Fakes2    = rebin(Fakes2   ,lNBins,lAxis);
    ///Rebin
...
CMSPrelim(const char* dataset, const char* channel,const char* cat)
{
  ...
  TPaveText* chan     = new TPaveText(lowX, lowY-0.002, lowX+0.45, lowY+0.028, "NDC");
...
TH1F* 
diffPlot(TH1F*h1,TH1F*h2,int opt)
{
...
float 
findMaxY(TH1F* h,int opt=0,float lowx=0.,float highx=0.)
{
  float max=0.;
...
float 
findMinY(TH1F* h,int opt=0,float lowx=0.,float highx=0.)
{
  float max=0.;
...
{ 
  ...
  if(tanb>0) {
    xmininset=0;//mass-100;  #felix
    xmaxinset=1000;//mass+100;  #felix
  }

  ////Format the histograms
  Ztt->GetYaxis()->SetRangeUser(0.,1.3*findMaxY(data,0));
  if(log) Ztt->GetYaxis()->SetRangeUser(0.001,80*findMaxY(data,0)); // felix 50*
  ...
  TGraphAsymmErrors* gr = 0;   //felix
  if(log){ 
    gr = new TGraphAsymmErrors();
    gr->SetPoint(0, errBand->GetBinCenter(0), 0.000);
    gr->SetPointEYlow(0, 0.001);
    gr->SetPointEYhigh(0, 0.001);
    for(Int_t b=1;b<=errBand->GetNbinsX();b++){
      gr->SetPoint(b, errBand->GetBinCenter(b), 0.001); 
      gr->SetPointEYlow(b, 0.0001);
      gr->SetPointEYhigh(b, errBand->GetBinError(b));
    }   
    gr->SetPoint(errBand->GetNbinsX()+1, errBand->GetBinCenter(errBand->GetNbinsX()+1), 0.001);
    gr->SetPointEYlow(errBand->GetNbinsX()+1, 0.001 );
    gr->SetPointEYhigh(errBand->GetNbinsX()+1, 0.001 );
  ...
  if(tanb>0) legendDiff.AddEntry((TObject*)0, mssmlabel, "");
  ...
  if(dataDiff->GetBinContent(dataDiff->FindBin(mass))<0){
    legendDiff.SetX1NDC(0.45);
    legendDiff.SetX2NDC(0.88);
    legendDiff.SetY1NDC(0.64); //felix 0.24 
    legendDiff.SetY2NDC(0.85); //felix 0.45 
  }
  ...
  pad.cd();
  if(log) pad.SetLogy(1); //felix
  pad.SetFillColor(0);
  pad.SetFillStyle(0);
  errBandFrame.Draw();
  if(log) errBandFrame.SetMaximum(7); //felix
  else errBandFrame.SetMaximum(3); //felix
  if(log) errBandFrame.SetMinimum(0.0001); //felix
  else errBandFrame.SetMinimum(-1); //felix
  if(log) gr->Draw("a3same"); //felix a3
  else errBand->Draw("e3lsame"); //felix 
  sig->Draw("hist same"); //hist same APH
  TLine line;
  if (!log) line.DrawLine(xmininset,0,xmaxinset,0);
