...
PlotLimits::print(const char* filename, std::vector<TGraph*> minus2sigma, std::vector<TGraph*> minus1sigma, std::vector<TGraph*> expected, std::vector<TGraph*> plus1sigma, std::vector<TGraph*> plus2sigma, std::vector<TGraph*> observed, double tanbLow, double tanbHigh, float *mass, const char* type)
{
  ...
  //per graph
  //if max < 1 -> all excluded
  //if min > 1 -> nothing excluded
  //all other cases scan from right(high tanb) to left(low tanb) 
  //start with "tanbmax - " 
  //if crossing at tanbX from <1 to >1 -> tanb>tanbX are excluded
  //"tanbmax - tanbX, "
  //if crossing at tanbY from >1 to <1 -> tanb<tanbX are excluded
  //"tanbmax - tanbX, tanbY - "
  //go on like that
  //if no other tanb regions are excluded finish
  //"tanbmax - tanbX, tanbY - tanbmin"
