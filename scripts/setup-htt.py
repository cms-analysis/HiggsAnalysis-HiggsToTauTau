...
parser.add_option("--category", dest="category", default="all", type="choice", help="Choose between event categories (depending on analysis) [Default: all]", choices=["cmb", "incl", "btag", "nobtag", "vbf", "boost", "all"])
...
if options.analysis == "sm" or options.analysis == "all":
    ...
    if options.channel == "cmb" or options.channel == "all":
        ...
        if options.category == "boost":
            ...
            os.system("setup-batch.py -n bosst  -c mt  -o htt_mt_1.txt -e SM-01    %s %s" % (optionals, options.sm_masses  ))
