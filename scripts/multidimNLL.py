...
def histcreation(path, asimov=""):
	...
	print "Global Minima for 2D model independent fits:\n", min(globalminformass)
	if options.nllOffSet == "":  nllOffSet = min(globalminformass)
	else: nllOffSet = -1*float(options.nllOffSet)

	if options.nllOffSet == "": NLLmuMin = NLLmuFixedforqmu.GetMinimum()
	else: NLLmuMin = -1*float(options.nllOffSet)
