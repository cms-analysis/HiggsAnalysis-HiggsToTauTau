...
parser.add_option(""    ,"--erroroption", dest="erroroption", default="0",       type="int",      help="Error option 0('legacy' analysis) 1(experimental) 2(new MSSM analysis) [Default:0]")
...
for cat in options.categories.split() :
    for bkg in options.background.split() : 
        status = os.system(r"root -l -b -q {CMSSW_BASE}/src/HiggsAnalysis/HiggsToTauTau/macros/addFitNuisance.C+\(\"{FILENAME}\"\,\"{CHANNEL}\"\,\"{BKG}\"\,\"{ENERGY}\"\,\"{NAME}\"\,\"{CATEGORY}\"\,{FIRST}\,{LAST}\,{FITOPTION}\,{FITMODEL}\,{ERROROPTION}\,{VERBOSE}\,{VARBIN}\,{UNCERTS}\,{TESTMODE}\)".format(
