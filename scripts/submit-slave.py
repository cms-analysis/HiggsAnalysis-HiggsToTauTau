    ...
    if not options.submitOnly :
        ...
        if options.binary == "combine" :
            ...
            if options.method == "tanb" :
                ...
                for tanb in points :
                    os.system("python tanb_grid.py -m {mass} -t {tanb} --model {model} --interpolation {interpolation} {model_independent} --BR {BR} tmp.txt".format(
                        mass=masspoint, tanb=tanb, model=options.model, interpolation=options.interpolation_mode, 
                        model_independent="--model-independent" if options.independent else "", BR=options.BR))
