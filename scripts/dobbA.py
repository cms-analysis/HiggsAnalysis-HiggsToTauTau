...
## configuration
config=configuration('bbA', options.config)
...
if options.update_setup :
    ...
    os.system("cp -r {CMSSW_BASE}/src/HiggsAnalysis/HiggsToTauTau/setup-bbA {CMSSW_BASE}/src/.setup{LABEL}".format(CMSSW_BASE=cmssw_base, LABEL=options.label))
    ## copy all input files in question and nothing else by these
    for chn in config.channels :
        print "... copy files for channel:", chn
        ## remove legacy
        for file in glob.glob("{SETUP}/{CHN}/*inputs-bbA-*.root".format(SETUP=setup, CHN=chn)) :
            os.system("rm %s" % file)
        for per in config.periods :
            ...
            for ana in analyses :
                ...
                if ana in patterns.keys() :
                    pattern =  ''
                source="{CMSSW_BASE}/src/auxiliaries/shapes/{DIR}/{CHN}.inputs-bbA-{PER}{PATTERN}.root".format(
                ...
                for file in glob.glob(source) :
                        os.system("cp -v {SOURCE} {SETUP}/{CHN}/{CHNNAME}.inputs-bbA-{PER}.root".format(
                            SOURCE=file,
                            CHN=chn,
                            CHNNAME='htt_'+chn,
                            SETUP=setup,
        ...
        for chn in config.channels:
            for per in config.periods:
                if config.categories[chn][per]:
                    os.system("setup-datacards.py -i {CMSSW_BASE}/src/setups{LABEL}/{ANA} -o {DIR}/{ANA} -p '{PER}' -a bbA -c '{CHN}' --bbA-categories-{CHN}='{CATS}' {MASSES}".format(
                        CMSSW_BASE=cmssw_base,
                        LABEL=options.label,
                        ANA=ana,
                        DIR=dir,
...
if options.update_limits :
    ...
    for ana in analyses :
        ...
        for chn in config.channels:
            for per in config.periods:
                if config.categories[chn][per]:
                    os.system("setup-bbA.py -i aux{INDEX}/{ANA} -o {DIR}/{ANA} -p '{PER}' -a bbA -c '{CHN}' {LABEL} --bbA-categories-{CHN}='{CATS}' {MASSES}".format(
                        INDEX=options.label,                
                        ANA=ana,
                        DIR=dir,
                        LABEL=label,
