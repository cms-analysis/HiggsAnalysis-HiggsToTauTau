...
bgroup.add_option("--toys", dest="toys", default="100", type="string",
                  help="Set number of toys of the median and quantiles for expected calculation. This is used in goodness of fit, significance, maximum likelihoood, feldman-cousins and hypothesis test. [Default: \"100\"]")
...
##
## FELDMAN-COUSINS OPTIONS
##
ngroup = OptionGroup(parser, "FELDMAN COUSINS COMMAND OPTIONS", "These are the command line options for the use of limits.py with option --feldman-cousins.")
...
for directory in args :
    ...
    if options.optGoodnessOfFit :
        ...
        if options.optCollect :
            ...
            os.system("hadd -f -k batch_collected_goodness_of_fit.root higgsCombineTest.GoodnessOfFit.mH{MASS}.[0-9]*.root".format(MASS=mass))
            ...
            if os.path.exists("old_goodness_of_fit.root") :
                ...
                os.system("hadd -k batch_collected_goodness_of_fit.root old_goodness_of_fit.root new_goodness_of_fit.root")
    ...
    if options.optAsym :
        ...
        if not options.expectedOnly :
            ...
            os.system("combine -M Asymptotic --run observed -C {cl} {minuit} --minimizerStrategy {strategy} -n '-obs' {qtilde} {mass} {user} {wdir}/{wsp}.root".format(
                cl=options.confidenceLevel, minuit=minuitopt, qtilde=qtildeopt, strategy=options.strategy, mass=massopt, user=options.userOpt, wdir=options.workingdir, wsp=wsp))
 ##    ##
##     ## INJECTED (compute expected dataset using toys)
##     ##
