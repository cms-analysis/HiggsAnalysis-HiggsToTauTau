...
agroup.add_option("--pvalue", dest="optPValue", default=False, action="store_true",
                  help="Calculate the observed and expected p-value from likelihood profiling. [Default: False]")
...
for directory in args :
    ...
    if options.optPValue :
        ...
        ## prepare workspace
        create_card_workspace(mass)
        ## calculate p-Value a la HCG
        if not options.observedOnly :
            ## calculate expected p-value
            print "combine -M ProfileLikelihood -n PVAL-exp --significance --pvalue --expectSignal=1 -t -1 --toysFreq -m {mass} {wdir}/tmp.root".format(mass=mass, wdir=options.workingdir)
            os.system("combine -M ProfileLikelihood -n PVAL-exp --significance --pvalue --expectSignal=1 -t -1 --toysFreq -m {mass} {wdir}/tmp.root".format(mass=mass, wdir=options.workingdir))
        if not options.expectedOnly :
            ## calculate expected p-value
            print "combine -M ProfileLikelihood -n PVAL-obs --significance --pvalue -m {mass} {wdir}/tmp.root".format(mass=mass, wdir=options.workingdir)
            os.system("combine -M ProfileLikelihood -n PVAL-obs --significance --pvalue -m {mass} {wdir}/tmp.root".format(mass=mass, wdir=options.workingdir))
    ##
    ## ASYMPTOTIC
    ##
    if options.optAsym :
