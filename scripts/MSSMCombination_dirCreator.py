    ...
    for mass in range(300, 3101, 100) :
        ...
        if os.path.exists("{HTT}/{MASS}/debug".format(HTT=options.path_htt, MASS=str(mass))) :
            ...
            for file in glob.glob("{HPLUS}/{MASS}/debug/*0.txt".format(HPLUS=options.path_hplus, MASS=str(mass))) :
                ...
                os.system("cp {HPLUS}/{MASS}/combine_histograms_hplushadronic_light.root_{MASS}_{TANB} LIMITS-MSSMCombination{LABEL}/{MASS}".format(HPLUS=options.path_hplus, LABEL="" if options.label=="" else "-"+options.label, MASS=str(mass), TANB=str(tanb[:-1])))
