...
mssm_xs_tools::mssm_xs_tools(const char* filename, bool kINT, unsigned verbosity) : verbosity_(verbosity), kINTERPOL_(kINT){
  if(verbosity>99){
    std::cout << "Welcome to the MSSM neutral cross section tool (v1.0)"
              << std::endl
              << " Monica Vazquez Acosta (Monica.Vazquez.Acosta@cern.ch)," << std::endl
              << " Trevor Vickey         (Trevor.Vickey@cern.ch),        " << std::endl
              << " Markus Warsinsky      (Markus.Warsinsky@cern.ch),     " << std::endl
              << " Felix Frensch         (FelixFrensch2@kit.edu)         " << std::endl;
    }
  if(!filename){
