...
void compareSignalStrength(const char* filename, const char* channelstr, const char* type, double minimum=0., double maximum=20., bool log=false, const char* label=" Preliminary, H#rightarrow#tau#tau, L=17 fb^{-1}", bool legendOnRight=true, bool legendOnTop=true)
{
  SetStyle();
  ...
  for(unsigned int i=0; i<hexp.size(); ++i){
	...
	x_title = std::string("m_{H} [GeV]");
      ...
      std::string y_title;
      if( std::string(type) == std::string("mssm") ){
	y_title = std::string("best fit for #sigma(#phi#rightarrow#tau#tau)");
      }
      else{
	//y_title = std::string("#sigma(H#rightarrow#tau#tau)_{95% CL} / #sigma(H#rightarrow#tau#tau)_{SM}");
	y_title = std::string("best fit for #sigma/#sigma_{SM}");
      }
  ...
  //if(std::string(type) == std::string("mssm")) {leg0->SetTextSize(0.03);}
  leg0->SetBorderSize( 0 );
  leg0->SetFillStyle ( 1001 );
  leg0->SetFillColor (kWhite);
  leg0->SetHeader( "#bf{Best Fit}" );
  for(unsigned int i=0; i<hexp.size(); ++i){
