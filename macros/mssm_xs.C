...
mssm_xs(char* path, double mA, double tanb)
{
  ...
  std::cout << " -> xsec(ggH)(tautau):\t" << xs.Give_Xsec_ggFH(mA, tanb) << "\t --- \t" << xs.Give_BR_H_tautau(mA, tanb) << "\t at mass: "  <<  xs.Give_Mass_H(mA, tanb) << std::endl;
  ...
  std::cout << " -> xsec(bbA)(tautau):\t" << xs.GiveXsec_Santander_A(mA, tanb) << "\t --- \t" << xs.Give_BR_A_tautau(mA, tanb) << std::endl;
