...
mssm_xs(char* path, double mA, double tanb)
{
  ...
  xs_eff_ggH_tautau[0] = xs.Give_Xsec_ggFA(mA, tanb)*xs.Give_BR_A_tautau(mA, tanb)/1000.;
  std::cout << " -> xsec(ggA)(tautau):\t" << xs.Give_Xsec_ggFA(mA, tanb)/1000. << "\t --- \t" << xs.Give_BR_A_tautau(mA, tanb) << std::endl;
  xs_eff_ggH_tautau[1] = xs.Give_Xsec_ggFH(mA, tanb)*xs.Give_BR_H_tautau(mA, tanb)/1000.;
  std::cout << " -> xsec(ggH)(tautau):\t" << xs.Give_Xsec_ggFH(mA, tanb)/1000. << "\t --- \t" << xs.Give_BR_H_tautau(mA, tanb) << std::endl;
  xs_eff_ggH_tautau[2] = xs.Give_Xsec_ggFh(mA, tanb)*xs.Give_BR_h_tautau(mA, tanb)/1000.;
  std::cout << " -> xsec(ggh)(tautau):\t" << xs.Give_Xsec_ggFh(mA, tanb)/1000. << "\t --- \t" << xs.Give_BR_h_tautau(mA, tanb) << std::endl;
  ...
  xs_eff_bbH_tautau[0] = xs.GiveXsec_Santander_A(mA, tanb)*xs.Give_BR_A_tautau(mA, tanb)/1000.;
  std::cout << " -> xsec(bbA)(tautau):\t" << xs.GiveXsec_Santander_A(mA, tanb)/1000. << "\t --- \t" << xs.Give_BR_A_tautau(mA, tanb) << std::endl;
  xs_eff_bbH_tautau[1] = xs.GiveXsec_Santander_H(mA, tanb)*xs.Give_BR_H_tautau(mA, tanb)/1000.;
  std::cout << " -> xsec(bbH)(tautau):\t" << xs.GiveXsec_Santander_H(mA, tanb)/1000. << "\t --- \t" << xs.Give_BR_H_tautau(mA, tanb) << std::endl;
  xs_eff_bbH_tautau[2] = xs.GiveXsec_Santander_h(mA, tanb)*xs.Give_BR_h_tautau(mA, tanb)/1000.;
  std::cout << " -> xsec(bbh)(tautau):\t" << xs.GiveXsec_Santander_h(mA, tanb)/1000. << "\t --- \t" << xs.Give_BR_h_tautau(mA, tanb) << std::endl;
