...
std::string legendEntry(const std::string& channel){
  ...
  if(channel==std::string("bbb_all_kinfitall"       )) title = std::string("bbb 2j0+1+2 (kinfit+mttbb)");
  if(channel==std::string("plain_all_kinfitall"       )) title = std::string("plain 2j0+1+2 (kinfit+mttbb)");
  if(channel==std::string("bbb_all"      )) title = std::string("bbb 2j0+1+2 (kinfit)");
  if(channel==std::string("plain_all"      )) title = std::string("Plain 2j0+1+2 (kinfit)");
...
void compareLimits(const char* filename, const char* channelstr, bool expected, bool observed, const char* type, double minimum=0., double maximum=20., bool log=false, const char* label="#scale[1.5]{CMS}   H#rightarrow#tau#tau  19.7 fb^{-1} (8 TeV)", bool legendOnRight=true, bool legendOnTop=true, bool ggH=true)
{
  ...
  colors["btag"       ] = kRed; 
  colors["nobtag"     ] = kBlue; 
  colors["plain_all_kinfitall"         ] = kBlue;
  colors["bbb_all_kinfitall"         ] = kRed;
  colors["plain_all"         ] = kGreen+3;
  colors["pt20"         ] = kMagenta;
  colors["pt30"         ] = kBlack;
  colors["bbb_all"         ] = kMagenta+3;
  colors["vhtt"       ] = kCyan-6;
  colors["cmb"        ] = kBlack;
  colors["htt"        ] = kBlack;
  colors["ggH"        ] = kRed;
